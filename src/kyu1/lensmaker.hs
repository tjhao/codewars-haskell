...
-- Control.Applicative.Const replicData.Traversable ated here for your
-- convenience
...
toListOf lens s = flip (appEndo . getK) [] $ lens (\a -> K $ Endo (a:)) s
...
preview lens = (getFirst . getK) . (lens $ K . First . Just)
...
coerce fa = (contramap . const) fa ffa
  where ffa = (fmap . const) fa fa
...
_Left = rmap g . left'
  where g (Right c) = pure $ Right c
        g (Left fb) = fmap Left fb
...
_Right = rmap g . right'
  where g (Left c) = pure $ Left c
        g (Right fb) = fmap Right fb
